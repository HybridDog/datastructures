...
setmetatable(Stack, {__call = function(_, data)
	local stack
	if type(data) == "table"
	and data.input then
		stack = data.input
...
setmetatable(Queue, {__call = function(_, data)
	local fifo
	if type(data) == "table"
	and data.input then
...
local BinaryHeap = {}
datastructures.BinaryHeap = BinaryHeap
setmetatable(BinaryHeap, {__call = function(_, data)
	local compare = data
	if type(data) == "table" then
		if data.input then
			...
			setmetatable(binary_heap, BinaryHeap)
